...
class DahuaClient:
    ...
    async def get_hardware_version(self) -> dict:
        """
        get_hardware_version returns the device hardware version
        """
        try:
            return await self.get("/cgi-bin/magicBox.cgi?action=getHardwareVersion")
        except aiohttp.ClientResponseError as e:
            return {"version": "unknown"}

    async def get_onvif_version(self) -> dict:
        """
        get_onvif_version returns the supported onvif version
        """
        try:
    ...
    async def get_http_api_version(self) -> dict:
        """
        get_http_api_version returns the supported cgi version
        """
        try:
            return await self.get(
    ...
    async def get_channels(self) -> dict:
        """get_channels returns list of channels titles"""
        try:
            return await self.get(
    ...
    async def fetch_file(
        self, channel, subtype, start_time, end_time, type, res_path
    ) -> dict:
        """fetch_file downloads a file from device"""
        try:
            return await self.download_file(
                "/cgi-bin/loadfile.cgi?action=startLoad&channel={}&startTime={}&endTime={}&subtype={}&Types={}".format(
                    channel, start_time, end_time, subtype, type
                ),
                res_path,
            )
        except aiohttp.ClientResponseError as e:
    ...
    async def async_get_general_config(self) -> dict:
        """async_get_general_config gets general config"""
    ...
    async def async_current_time(self) -> dict:
        """async_current_time gets current device time"""
        url = "/cgi-bin/global.cgi?action=getCurrentTime"
        try:
            return await self.get(url)
        except aiohttp.ClientResponseError as e:
            return {}
    ...
    async def download_file(self, url: str, save_path: str):
        """
        Асинхронно загружает файл по указанному URL и сохраняет его на диск.
        :param url: URL файла для загрузки
        :param save_path: Путь для сохранения файла
        """
        url = self._base + url
        try:
            async with async_timeout.timeout(FILE_TIMEOUT_SECONDS):
                response = None
                try:
                    auth = DigestAuth(self._username, self._password, self._session)
                    response = await auth.request("GET", url)
            ...
            _LOGGER.warning("TimeoutError fetching file from %s", url)
            raise exception
        except (KeyError, TypeError) as exception:
            _LOGGER.warning("TypeError fetching file from %s", url)
            raise exception
        except (aiohttp.ClientError, socket.gaierror) as exception:
            _LOGGER.debug("ClientError fetching file from %s", url)
            raise exception
        except Exception as exception:  # pylint: disable=broad-except
            _LOGGER.warning("Exception fetching file from %s", url)
            raise exception
